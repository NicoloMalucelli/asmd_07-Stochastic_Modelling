...
object StochasticReadersAndWriters extends App:
  ...
  val rw = SPN[Place](
    Trn(MSet(IDLE), m => 0.01,   MSet(SELECT),  MSet()),
    Trn(MSet(SELECT), m => 100_000,  MSet(WAIT_FOR_READING),  MSet()),
    Trn(MSet(SELECT), m => 200_000,   MSet(WAIT_FOR_WRITING),   MSet()),
  ...
  val sim = toCTMC(rw).simulateOnce(MSet.ofMap(Map((IDLE, k), (TOKEN, 1))), new Random)(300)
  print(sim.size)
  println(
